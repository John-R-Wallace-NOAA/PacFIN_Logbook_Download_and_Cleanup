...
LBData.1981.2018 <- NULL 
for ( i in 1981:2018) 
       LBData.1981.2018 <- rbind(LBData.1981.2018, PacFIN.Logbook.Data.by.Year(i, uid = PacFIN.Login, pwd = PacFIN.PW)[,PacFinCols]) # Downloading a few colums that are not used 

       LBData.2016 <- PacFIN.Logbook.Data.by.Year(2016, uid = PacFIN.Login, pwd = PacFIN.PW)[,PacFinCols]
       
# Saved again later with bimo and Key columns and other changes, saved the raw data here because of the long download time. 
save(LBData.1981.2018, file = "Funcs and Data/LBData.1981.2018.dmp")  
      ...
      # base::load('Funcs and Data/LBData.1981.2016.dmp')
      dim(LBData.1981.2016)
      LBData.1981.2016 <- rbind(LBData.1981.2016, Test.2016)
      }

dim(LBData.1981.2018)
head(LBData.1981.2018)
...
Table(LBData.1981.2018$RYEAR, LBData.1981.2018$AGID)  # JRWToolBox::Table() function defaults to show the NA's, unlike table(). Seeing the NA's is alsmost always important in fisheries work.
...
# base::load('Funcs and Data/LBData.1981.2018.dmp') 
# Table(LBData.1981.2018$RYEAR, LBData.1981.2018$AGID) - Table(LBData.1981.2018$RYEAR, LBData.1981.2018$AGID)
...
LBData.1981.2018$DDATE <- as.Date(LBData.1981.2018$DDATE, "%d-%b-%Y")
LBData.1981.2018$RDATE <- as.Date(LBData.1981.2018$RDATE, "%d-%b-%Y")
LBData.1981.2018$bimo <- ceiling(as.numeric(recode.simple(months(LBData.1981.2018$RDATE, T) , cbind(c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"), 1:12)))/2)
LBData.1981.2018$SET_LONG <- - LBData.1981.2018$SET_LONG
LBData.1981.2018$UP_LONG <- - LBData.1981.2018$UP_LONG

# Create a unique tow key 
LBData.1981.2018$Key <- paste(LBData.1981.2018$AGID, LBData.1981.2018$TRIP_ID, LBData.1981.2018$TOWNUM)  
length(unique(LBData.1981.2018$Key)) # 1,145,548 (1,114,030 for 1981 - 2015)

# # Add even more items to a test key to check uniqeness
TestKey <- paste(LBData.1981.2018$AGID, LBData.1981.2018$TRIP_ID, LBData.1981.2018$TOWNUM, LBData.1981.2018$RYEAR, LBData.1981.2018$DRVID) 
length(unique(TestKey))  # 1,145,548 - same as above (1,114,030  for 1981 - 2015) 
rm(TestKey)
