...
# - In the end, block centroid lat/longs were only used for missing Oregon data, but that adds 21,949 more tows with lat/long (see below)
###########################


# base::load("Funcs and Data/LB Polygons Dat 25 Mar 2019.RData") # Load if needed
# load("Funcs and Data/EEZ.Polygon.WestCoast.RData") # Load if needed

# Centroids of logbook blocks - from Marlene Bellman
# JRWToolBox::gitAFile("https://cdn.jsdelivr.net/gh/John-R-Wallace/PacFIN_Logbook_Download_and_Cleanup@master/R/Funcs and Data/LogBookBlocks.RData", type = 'RData', File = "Funcs and Data/LogBookBlocks.RData")
load("Funcs and Data/LogBookBlocks.RData") 
...
# Match the block centroids into Dat by block and rename
...
# For the California agency data off of California, most of the missing lat/long tows were assigned to a single local block with average longitude 
    ...
    # Figure for 1999 showing that the block numbers (expressed as block centroids) given to CA tows off of OR are not correct
    dev.new()
    ilines(list(world.h.land, world.h.borders, EEZ.Polygon.WestCoast), c(-135, -116), c(29.5, 49.5), zoom = F)
...
# ********** Given the above issues with CA block data, CA was removed from the best lat/long when the lat/long were NA **********
...
# Dat is not saved here - please move to the next step
